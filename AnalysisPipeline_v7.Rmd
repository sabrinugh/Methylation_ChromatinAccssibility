...
## Section 1: Cleaning data values for analysis
...
## Section 3: Plot the preliminary analysis
...
## Section 4: Methylation and ATAC-seq annotation subsetting
...
## Section 5: Plotting out Annotations via circos plots
Utilising the 'circlize' library (reference accordingly)

``` {r Function Def: Comparison between methylation and ATAC-seq peak locations for a given genomic feature}
...
## Section 6: Methylation and Accessibility and varying points of distribution
...
matchedAvgMethIntrons <- setNames(rep(0, length(methIntronsCpGs)), methIntronsCpGs)
matchedAvgMethIntrons[mutualMethIntrons$IDs] <- avgMethIntrons
matchedAvgAtacIntrons <- setNames(rep(0, length(atacIntronsPeaks)), atacIntronsPeaks)
matchedAvgAtacIntrons[mutualAtacIntrons$IDs] <- avgAtacIntrons

# Exons
methExonsCpGs <- methExonsEqual$names; atacExonsPeaks <- atacExons$names
matchedAvgMethExons <- setNames(rep(0, length(methExonsCpGs)), methExonsCpGs)
matchedAvgMethExons[mutualMethExons$IDs] <- avgMethExons
matchedAvgAtacExons <- setNames(rep(0, length(atacExonsPeaks)), atacExonsPeaks)
matchedAvgAtacExons[mutualAtacExons$IDs] <- avgAtacExons

# Promoters
methPromotersCpGs <- methPromotersEqual$names; atacPromotersPeaks <- atacPromoters$names
matchedAvgMethPromoters <- setNames(rep(0, length(methPromotersCpGs)), methPromotersCpGs)
matchedAvgMethPromoters[mutualMethPromoters$IDs] <- avgMethPromoters
matchedAvgAtacPromoters <- setNames(rep(0, length(atacPromotersPeaks)), atacPromotersPeaks)
matchedAvgAtacPromoters[mutualAtacPromoters$IDs] <- avgAtacPromoters

# Distal Intergenic (DistalInt)
methDistalIntCpGs <- methDistalIntEqual$names; atacDistalIntPeaks <- atacDistalInt$names
matchedAvgMethDistalInt <- setNames(rep(0, length(methDistalIntCpGs)), methDistalIntCpGs)
matchedAvgMethDistalInt[mutualMethDistalInt$IDs] <- avgMethDistalInt
matchedAvgAtacDistalInt <- setNames(rep(0, length(atacDistalIntPeaks)), atacDistalIntPeaks)
matchedAvgAtacDistalInt[mutualAtacDistalInt$IDs] <- avgAtacDistalInt

# UTRs ('3 and '5)
methUTR3CpGs <- methUTR3Equal$names; atacUTR3Peaks <- atacUTR3$names
matchedAvgMethUTR3 <- setNames(rep(0, length(methUTR3CpGs)), methUTR3CpGs)
matchedAvgMethUTR3[mutualMethUTR3$IDs] <- avgMethUTR3
matchedAvgAtacUTR3 <- setNames(rep(0, length(atacUTR3Peaks)), atacUTR3Peaks)
matchedAvgAtacUTR3[mutualAtacUTR3$IDs] <- avgAtacUTR3

methUTR5CpGs <- methUTR5Equal$names; atacUTR5Peaks <- atacUTR5$names
matchedAvgMethUTR5 <- setNames(rep(0, length(methUTR5CpGs)), methUTR5CpGs)
matchedAvgMethUTR5[mutualMethUTR5$IDs] <- avgMethUTR5
matchedAvgAtacUTR5 <- setNames(rep(0, length(atacUTR5Peaks)), atacUTR5Peaks)
matchedAvgAtacUTR5[mutualAtacUTR5$IDs] <- avgAtacUTR5

# Downstream
methDownstreamCpGs <- methDownstreamEqual$names; atacDownstreamPeaks <- atacDownstream$names
matchedAvgMethDownstream <- setNames(rep(0, length(methDownstreamCpGs)), methDownstreamCpGs)
matchedAvgMethDownstream[mutualMethDownstream$IDs] <- avgMethDownstream
matchedAvgAtacDownstream <- setNames(rep(0, length(atacDownstreamPeaks)), atacDownstreamPeaks)
matchedAvgAtacDownstream[mutualAtacDownstream$IDs] <- avgAtacDownstream
...
mcols(methIntronsEqual)$avg <- matchedAvgMethIntrons
mcols(atacIntrons)$avg <- matchedAvgAtacIntrons

mcols(methExonsEqual)$avg <- matchedAvgMethExons
mcols(atacExons)$avg <- matchedAvgAtacExons

mcols(methPromotersEqual)$avg <- matchedAvgMethPromoters
mcols(atacPromoters)$avg <- matchedAvgAtacPromoters

mcols(methDistalIntEqual)$avg <- matchedAvgMethDistalInt
mcols(atacDistalInt)$avg <- matchedAvgAtacDistalInt

mcols(methUTR3Equal)$avg <- matchedAvgMethUTR3
mcols(atacUTR3)$avg <- matchedAvgAtacUTR3

mcols(methUTR5Equal)$avg <- matchedAvgMethUTR5
mcols(atacUTR5)$avg <- matchedAvgAtacUTR5

mcols(methDownstreamEqual)$avg <- matchedAvgMethDownstream
mcols(atacDownstream)$avg <- matchedAvgAtacDownstream
```
...
``` {r Normal sample methylation}
normalMeth$IDs <- methCpGIDs
normalMethIntrons <- normalMeth[methCpGIDs %in% methIntronsEqual$names, ] 
normalMethIntronsMatrix <- as.matrix(normalMethIntrons[, .SD, .SDcols = sapply(normalMethIntrons, is.numeric)])

normalMethExons <- normalMeth[methCpGIDs %in% methExonsEqual$names, ] 
normalMethExonsMatrix <- as.matrix(normalMethExons[, .SD, .SDcols = sapply(normalMethExons, is.numeric)])

normalMethPromoters <- normalMeth[methCpGIDs %in% methPromotersEqual$names, ] 
normalMethPromotersMatrix <- as.matrix(normalMethPromoters[, .SD, .SDcols = sapply(normalMethPromoters, is.numeric)])

normalMethDistalInt <- normalMeth[methCpGIDs %in% methDistalIntEqual$names, ] 
normalMethDistalIntMatrix <- as.matrix(normalMethDistalInt[, .SD, .SDcols = sapply(normalMethDistalInt, is.numeric)])

normalMethUTR3 <- normalMeth[methCpGIDs %in% methUTR3Equal$names, ] 
normalMethUTR3Matrix <- as.matrix(normalMethUTR3[, .SD, .SDcols = sapply(normalMethUTR3, is.numeric)])

normalMethUTR5 <- normalMeth[methCpGIDs %in% methUTR5Equal$names, ] 
normalMethUTR5Matrix <- as.matrix(normalMethUTR5[, .SD, .SDcols = sapply(normalMethUTR5, is.numeric)])

normalMethDownstream <- normalMeth[methCpGIDs %in% methDownstreamEqual$names, ] 
normalMethDownstreamMatrix <- as.matrix(normalMethDownstream[, .SD, .SDcols = sapply(normalMethDownstream, is.numeric)])


normalMeanIntrons <- rowMeans(normalMethIntronsMatrix)
normalMeanExons <- rowMeans(normalMethExonsMatrix)
normalMeanPromoters <- rowMeans(normalMethPromotersMatrix)
normalMeanDistalInt <- rowMeans(normalMethDistalIntMatrix)
normalMeanUTR3 <- rowMeans(normalMethUTR3Matrix)
normalMeanUTR5 <- rowMeans(normalMethUTR5Matrix)
normalMeanDownstream <- rowMeans(normalMethDownstreamMatrix)

normalIntronsMean <- setNames(rep(0, length(methIntronsCpGs)), methIntronsCpGs)
normalIntronsMean[normalMethIntrons$IDs] <- normalMeanIntrons
normalExonsMean <- setNames(rep(0, length(methExonsCpGs)), methExonsCpGs)
normalExonsMean[normalMethExons$IDs] <- normalMeanExons
normalPromotersMean <- setNames(rep(0, length(methPromotersCpGs)), methPromotersCpGs)
normalPromotersMean[normalMethPromoters$IDs] <- normalMeanPromoters
normalDistalIntMean <- setNames(rep(0, length(methDistalIntCpGs)), methDistalIntCpGs)
normalDistalIntMean[normalMethDistalInt$IDs] <- normalMeanDistalInt
normalUTR3Mean <- setNames(rep(0, length(methUTR3CpGs)), methUTR3CpGs)
normalUTR3Mean[normalMethUTR3$IDs] <- normalMeanUTR3
normalUTR5Mean <- setNames(rep(0, length(methUTR5CpGs)), methUTR5CpGs)
normalUTR5Mean[normalMethUTR5$IDs] <- normalMeanUTR5
normalDownstreamMean <- setNames(rep(0, length(methDownstreamCpGs)), methDownstreamCpGs)
normalDownstreamMean[normalMethDownstream$IDs] <- normalMeanDownstream

normalIntrons <- methIntronsEqual
normalExons <- methExonsEqual
normalPromoters <- methPromotersEqual
normalDistalInt <- methDistalIntEqual
normalUTR3 <- methUTR3Equal
normalUTR5 <- methUTR5Equal
normalDownstream <- methDownstreamEqual

mcols(normalIntrons)$avgMeth <- normalIntronsMean
mcols(normalExons)$avgMeth <- normalExonsMean
mcols(normalPromoters)$avgMeth <- normalPromotersMean
mcols(normalDistalInt)$avgMeth <- normalDistalIntMean
mcols(normalUTR3)$avgMeth <- normalUTR3Mean
mcols(normalUTR5)$avgMeth <- normalUTR5Mean
mcols(normalDownstream)$avgMeth <- normalDownstreamMean
```

``` {r Function Def: Single GRange Obj Plotting}
# Normal methylation plotting
featureNormalCircosValues <- function(grObj, featureName = "Feature") {
...
------------
### Overlay Methylation and ATAC-seq means on the genome

``` {r Function Def: Overlaying for easier comparison}
...
## Section 7: BRCA Subtypes
``` {r}
# Read in the excel data, with the subsetted BRCA types and their sample names
brcaSamples <- read_excel("Data/ATAC_DonorsAndStats_v4.xlsx", sheet = "Sheet2")[,c(1, 7)]
...
features <- c("Exons", "Introns", "Promoters", "DistalInt", "UTR3", "UTR5", "Downstream")

splitFeatures(features, subgroups)
```

``` {r}
# With subgroups split into features
# Now we can calculate the mean of the rows
featureMeans <- function(dataType = c("meth", "atac"), subgroups, features) {
  for (feat in features) {
    sites <- if (dataType == "meth") {get(paste0(dataType, feat, "CpGs"))} else { get(paste0(dataType, feat, "Peaks"))}
    for (sub in subgroups) {
...
### Plotting out
``` {r Rewrite of Overlay specific for the subtypes}
...
``` {r Meth/ATAC subtype comparison using the rewritten overlay function}
# Using Circos plots as with before to compare the methylation and atac-seq data
# Note: use for statements otherwise it's too messy 
for (sub in subgroups) {
...
## Section 8: KMT2C/D Chromosome Focus
Focusing on the chromosomes where KMT2C/D are based in humans (chromosome 7 and 12 respectively)
First idea, plot it out as a circos plot for each significant feature (i.e. exons, introns, promoters and the distal intergenic region).
Second idea, plot it out as a line plot. Use x-axis as the cpg/peak location of the chromosome, and the y-axis to denote the normalised value of methylation/chromatin accessibility for each significant feature.
...
```{r PLotting out Subtypes in both methylation and atac-seq}
## Introns
layout(matrix(1:4, 2, 2))
featureComparisonSubtypeOverlay(methIntronsGenesLumA, atacIntronsGenesLumA, "Introns", "LumA")
featureComparisonSubtypeOverlay(methIntronsGenesLumB, atacIntronsGenesLumB, "Introns", "LumB")
featureComparisonSubtypeOverlay(methIntronsGenesBasal, atacIntronsGenesBasal, "Introns", "Basal")
#featureComparisonSubtypeOverlay(methIntronsGenesNormal, atacIntronsGenesNormal, "Introns", "Normal")
# featureComparisonSubtypeOverlay(methIntronsGenesHer2, atacIntronsGenesHer2, "Introns", "Her2")

## Exons
layout(matrix(1:4, 2, 2))
featureComparisonSubtypeOverlay(methExonsGenesLumA, atacExonsGenesLumA, "Exons", "LumA")
featureComparisonSubtypeOverlay(methExonsGenesLumB, atacExonsGenesLumB, "Exons", "LumB")
featureComparisonSubtypeOverlay(methExonsGenesBasal, atacExonsGenesBasal, "Exons", "Basal")
#featureComparisonSubtypeOverlay(methExonsGenesNormal, atacExonsGenesNormal, "Exons", "Normal")
#featureComparisonSubtypeOverlay(methExonsGenesHer2, atacExonsGenesHer2, "Exons", "Her2")

## Promoters
layout(matrix(1:4, 2, 2))
featureComparisonSubtypeOverlay(methPromotersGenesLumA, atacPromotersGenesLumA, "Promoters", "LumA")
featureComparisonSubtypeOverlay(methPromotersGenesLumB, atacPromotersGenesLumB, "Promoters", "LumB")
featureComparisonSubtypeOverlay(methPromotersGenesBasal, atacPromotersGenesBasal, "Promoters","Basal")
#featureComparisonSubtypeOverlay(methPromotersGenesNormal, atacPromotersGenesNormal, "Promoters", "Normal")
#featureComparisonSubtypeOverlay(methPromotersGenesHer2, atacPromotersGenesHer2, "Promoters", "Her2")

## Distal intergenic
layout(matrix(1:4, 2, 2))
featureComparisonSubtypeOverlay(methDistalIntGenesLumA, atacDistalIntGenesLumA, "DistalInt", "LumA")
featureComparisonSubtypeOverlay(methDistalIntGenesLumB, atacDistalIntGenesLumB, "DistalInt", "LumB")
featureComparisonSubtypeOverlay(methDistalIntGenesBasal, atacDistalIntGenesBasal, "DistalInt", "Basal")
#featureComparisonSubtypeOverlay(methDistalIntGenesNormal, atacDistalIntGenesNormal, "Distal Intergenic", "Normal")
#featureComparisonSubtypeOverlay(methDistalIntGenesHer2, atacDistalIntGenesHer2, "DistalInt", "Her2")


# Methylation only
layout(matrix(1:4, 2, 2))
featureNormalCircosValues(methIntronsGenesLumA, "LumA")
featureNormalCircosValues(methIntronsGenesLumB, "LumB")
featureNormalCircosValues(methIntronsGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(methExonsGenesLumA, "LumA")
featureNormalCircosValues(methExonsGenesLumB, "LumB")
featureNormalCircosValues(methExonsGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(methPromotersGenesLumA, "LumA")
featureNormalCircosValues(methPromotersGenesLumB, "LumB")
featureNormalCircosValues(methPromotersGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(methDistalIntGenesLumA, "LumA")
featureNormalCircosValues(methDistalIntGenesLumB, "LumB")
featureNormalCircosValues(methDistalIntGenesBasal, "Basal")

# ATAC-seq Only
layout(matrix(1:4, 2, 2))
featureNormalCircosValues(atacIntronsGenesLumA, "LumA")
featureNormalCircosValues(atacIntronsGenesLumB, "LumB")
featureNormalCircosValues(atacIntronsGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(atacExonsGenesLumA, "LumA")
featureNormalCircosValues(atacExonsGenesLumB, "LumB")
featureNormalCircosValues(atacExonsGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(atacPromotersGenesLumA, "LumA")
featureNormalCircosValues(atacPromotersGenesLumB, "LumB")
featureNormalCircosValues(atacPromotersGenesBasal, "Basal")

layout(matrix(1:4, 2, 2))
featureNormalCircosValues(atacDistalIntGenesLumA, "LumA")
featureNormalCircosValues(atacDistalIntGenesLumB, "LumB")
featureNormalCircosValues(atacDistalIntGenesBasal, "Basal")
```
...
``` {r Comparison of Chr 7 and 12 with Mutual Samples between both ATAC-seq and methyaltion}
for (feat in features) {
...
layout(matrix(1:4, 2, 2))
featureComparisonCircosValuesOverlay(mutualMethIntronsGenes, mutualAtacIntronsGenes, "Introns")
featureComparisonCircosValuesOverlay(mutualMethExonsGenes, mutualAtacExonsGenes, "Exons")
featureComparisonCircosValuesOverlay(mutualMethPromotersGenes, mutualAtacPromotersGenes, "Promoters")
featureComparisonCircosValuesOverlay(mutualMethDistalIntGenes, mutualAtacDistalIntGenes, "DistalInt")

# Methylation only
layout(matrix(1:4, 2, 2))
featureNormalCircosValues(mutualMethIntronsGenes, "Introns")
featureNormalCircosValues(mutualMethExonsGenes, "Exons")
featureNormalCircosValues(mutualMethPromotersGenes, "Promoters")
featureNormalCircosValues(mutualMethDistalIntGenes, "DistalInt")

# ATAC-seq only
layout(matrix(1:4, 2, 2))
featureNormalCircosValues(mutualAtacIntronsGenes, "Introns")
featureNormalCircosValues(mutualAtacExonsGenes, "Exons")
featureNormalCircosValues(mutualAtacPromotersGenes, "Promoters")
featureNormalCircosValues(mutualAtacDistalIntGenes, "DistalInt")
