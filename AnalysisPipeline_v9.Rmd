...
medianPrimary <- median(avgPrimary, na.rm= T) # 0.5072542
medianNormal <- median(avgNormal, na.rm = T) # 0.5191214
medianMetastatic <- median(avgMetastatic, na.rm = T) # 0.5070332
...
methProbe <- methProbe[, c("chrom", "chromStart", "chromEnd"), ]
atacProbe <- atacProbe[complete.cases(atacProbe[, c("chrom", "chromStart", "chromEnd")]), ]
...
# Variable format: methIntronEqual, atacExon, etc.
```
...
findKMT2GenesSubs("meth", subgroups, features)
findKMT2GenesSubs("atac", subgroups, features)
...
Using Pearson's correlation tests to find the correlation between methylation and ATAC-seq. 
Focus on the average methylation and average chromatin accessibility of primary tumour samples across the genome for all features.

```{r}
for (feat in sigFeatures) {
  methGR <- get(paste0("meth", feat, "Genes"))
  atacGR <- get(paste0("atac", feat, "Genes"))
  
  methGeneAvgs <- tapply(mcols(methGR)$avg, mcols(methGR)$SYMBOL, mean, na.rm = TRUE)
  atacGeneAvgs <- tapply(mcols(atacGR)$avg, mcols(atacGR)$SYMBOL, mean, na.rm = TRUE)
  
  commonGenes <- intersect(names(methGeneAvgs), names(atacGeneAvgs))
  
  methVals <- methGeneAvgs[commonGenes]
  atacVals <- atacGeneAvgs[commonGenes]
  
  cor.test(methVals, atacVals, method = "pearson")
}


methGeneAvgs <- tapply(mcols(methPromotersGenes)$avg, mcols(methPromotersGenes)$SYMBOL, mean, na.rm = TRUE)
atacGeneAvgs <- tapply(mcols(atacPromotersGenes)$avg, mcols(atacPromotersGenes)$SYMBOL, mean, na.rm = TRUE)
...
```{r Methylation v. ATAC-seq correlation by sample}
# For each mutual sample: Find the avg methylation/chromatin accessibility for the KMT2C and KMT2D sites  (i.e. for all mutualMeth and mutualAtac)
mutualMeth[methCpGIDs %in% methIntronsGenes$names, ]
mutualATAC[peakIDs %in% atacIntronsGenes$names, ]


# Given above works
# Get colMeans for the samples, input into another dataframe to hold all potentials. Not necessary to normalise as it's all ratio rather than actual value. But could potentially normalise to ensure the consistency (make sure I'm not fucking it up)
# Don't remove non-complete cases, contains more sites in the mutualMeth as shown in for KMT2C/D presence. Otherwise, using the filtered version removes a lot of valuable information

# Pearson's is scale-invariant, so no need to normalise
corResults <- list()
# corRes <- list()

# Pearsons for the KMT2C/D genes present in the common BRCA samples
for (feat in sigFeatures) {
  methIDs <- get("methCpGIDs")
  atacIDs <- get("peakIDs")
  
  methGR <- get(paste0("meth", feat, "Genes"))
  atacGR <- get(paste0("atac", feat, "Genes"))
  
  temp <- mutualMeth[methIDs %in% methGR$names, ]
  test1 <- colMeans(temp[, -1, with = F], na.rm = T)
  temp <- mutualATAC[atacIDs %in% atacGR$names, ]
  test2 <- colMeans(temp[, -1, with = F], na.rm = T)
  
  corResults[[feat]] <- cor.test(test1, test2, method = "pearson")
  # corRes[[feat]] <- cor(test1, test2) # Default Pearson test
}
corResults
...
for (feat in sigFeatures) {
  ...
  methGR <- get(paste0("meth", feat, "Equal"))
  atacGR <- get(paste0("atac", feat))
